...
class BookDaoClass
{
    ...
    {
        $stmt = $this->connection->prepare("insert into Book (title, isRead, grade) values (:title, :isRead, :grade)");
        $stmt2 = $this->connection->prepare("insert into BookAuthor (bookId, authorId) values (:bookId, :authorId)");
    ...
    {
        $stmt1 = $this->connection->prepare("delete from BookAuthor where bookId = :id");
        $stmt1->bindValue(":id", $id);
        $stmt1->execute();

        $stmt2 = $this->connection->prepare("delete from Book where Book.id = :id");
    ...
    {
        $stmt = $this->connection->prepare("update Book
                                            set title = :title,
                                            isRead = :isRead,
                                            grade = :grade 
                                            where Book.id = :id");
...
//           and BookAuthor.authorId != :authorId");
        ...
        $stmt3 = $this->connection->prepare("insert into BookAuthor (bookId, authorId) values (:bookId, :authorId)");
        foreach ($book->authors as $author) {
    ...
    {
        $stmt = $this->connection->prepare("select Book.Id as bookId, title, Book.grade as bookGrade, Book.isRead,
        ...
        foreach ($stmt as $each) {
            ...
            if (!$added) {
                $newBook = new Book($each["title"], $each["isRead"], $each["bookGrade"], $each["bookId"]);
